...
\chapter{Repaso Estadística Descriptiva e Inferencial}\label{repaso-estaduxedstica-descriptiva-e-inferencial}
...
\section{Sesgo por Variable Omitida}\label{sesgo-por-variable-omitida}

Dentro de nuestros modelos, podríamos tener distintos problemas, uno de ellos es el ``Sesgo por Variable Omitida''. Esto ocurre cuando nosotros, como bien dice el nombre, omitimos una variable que es importante en el estudio, y está correlacionada con las otras variables y con la variable regresada. Para ilustrarlo, imaginemos ``conocemos'' un modelo poblacional que está estimado de tal forma que:

\[
Y = \gamma_0 + \gamma_1X_1 + \gamma_2X_2 + v
\]
Y nosotros, de manera muestral estimamos el siguiente modelo:

\[
Y = \beta_0 + \beta_1X_1 + \varepsilon
\]
Sabiendo que el estimador de \(\widehat{\beta_1}\) corresponde a \(\widehat{\beta_1} = \frac{Cov(Y,X_1)}{V(X_1)}\), entonces estudiamos insesgadez con el modelo poblacional que ``conocemos'':

\[
E[\widehat{\beta_1}|X] = E\left[\frac{Cov(\gamma_0 + \gamma_1X_1 + \gamma_2X_2 + v,X)}{V(X_1)}|X\right]
\]

\[
E[\widehat{\beta_1}|X] = E\left[\frac{Cov(\gamma_0,X) + \gamma_1Cov(X_1,X_1) + \gamma_2Cov(X_2,X_1) + Cov(v,X)}{V(X_1)}|X\right]
\]
Sabiendo que la covarianza entre una constante y una variable es 0, la covarianza del error con una variable es cero por el supuesto de media condicional del error y la covarianza entre una variable y si misma es su varianza, se obtiene:
\[
E[\widehat{\beta_1}|X] = E\left[\gamma_1 + \gamma_2\frac{Cov(X_1,X_2)}{V(X_1)}\right]
\]

Todo lo que tiene \(X\) y, todo lo constante sale hacia afuera de la esperanza, tal que:

\[
E[\widehat{\beta_1}|X] = \gamma_1 + \gamma_2\frac{Cov(X_1,X_2)}{V(X_1)}
\]

Entonces, queda demostrado que omitir una variable relevante, que covaría con otra de las variables regresoras genera un sesgo en el estimador de \(\widehat{\beta_1}\) ya que, el efecto de la variable omitida se está capturando por \(\widehat{\beta_1}\) y por ende se está sobreestimando o subestimando el efecto de dicha variable.
...
\section{Parte I: Comentes}\label{parte-i-comentes}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  En el modelo de regresión simple yi = β0 + β1xi + ui, si la varianza muestral de
...
\item
  En el contexto del modelo de regresión simple, que el estimador MCO de β1
  sea insesgado implica necesariamente que también es el mejor estimador lineal insesgado
  (MELI).
\item
  Un un modelo log-log de la forma \(ln(y_i) = \beta_0 + \beta_1ln(x_i) + u_i\), el coeficiente \(\hat{\beta_1}\)
  mide el cambio en y (en las unidades de y) cuando x aumenta en una unidad.
\item
  Un investigador analiza datos de comunas chilenas y encuentra una correlación
...
\section{Parte II: Matemáticos}\label{parte-ii-matemuxe1ticos}

\subsection{Modelos de Regresión Lineal Simple}\label{modelos-de-regresiuxf3n-lineal-simple}

\subsection{Modelos de Regresión Lineal Múltiple}\label{modelos-de-regresiuxf3n-lineal-muxfaltiple}

\subsection{Modelos con Variables Dummy}\label{modelos-con-variables-dummy}

\subsection{Modelos con Heterocedasticidad}\label{modelos-con-heterocedasticidad}

\subsection{Modelos con Endogeneidad}\label{modelos-con-endogeneidad}
