...
```{r, echo=FALSE, fig.width=7, fig.height=4.5, out.width="100%", fig.align="center"}
library(ggplot2)
...
```{r, echo = FALSE, fig.width=7, fig.height=4.5, out.width="100%", fig.align="center"}
# 1. Definir la función matemática del VIF
vif_funcion <- function(x) { 1 / (1 - x) }

# 2. Graficar la curva aproximándose al límite
curve(vif_funcion, 
      from = 0, 
      to = 0.99,       # Nos acercamos al límite matemático sin tocar el 1
      col = "royalblue", 
      lwd = 3,
      ylim = c(0, 30), # Ampliamos el eje Y para ver la subida vertical
      main = "Comportamiento del VIF",
      xlab = expression(R^2 ~ "(Colinealidad entre predictores)"),
      ylab = "Valor del VIF",
      panel.first = grid()) # Añade cuadrícula de fondo

# 3. ASÍNTOTA VERTICAL CRÍTICA (En R² = 1)
# El argumento 'v = 1' dibuja la línea vertical exacta en el valor 1 del eje X
abline(v = 1, col = "firebrick", lty = 2, lwd = 2.5)

# 4. Texto explicativo de la asíntota
text(x = 0.95, y = 25, 
     labels = "Asíntota en R² = 1\n(VIF tiende al Infinito)", 
     col = "firebrick", 
     font = 2, 
     pos = 2) # pos=2 alinea el texto a la izquierda de la coordenada

# 5. Asíntota horizontal base (VIF = 1)
abline(h = 1, col = "darkgreen", lty = 3, lwd = 1.5)
text(x = 0.2, y = 2, "Mínimo absoluto (VIF = 1)", col = "darkgreen", font = 3)
